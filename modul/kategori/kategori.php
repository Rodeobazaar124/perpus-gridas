...
if (empty($_SESSION['username']) and empty($_SESSION['passuser'])) {
 ...
 <center>Untuk mengakses modul, Anda harus login <br>";
  ...
  <h4 align=center>Referensi kategori</h4>
  <hr>
  <a href='?modul=kategori&act=tambahkategori'>
  <button class='btn btn-info' style='margin-left:10px;' type='button'>Tambah kategori</button></a>
  <hr>
		...
		<thead>
          <tr><th width='50px'>No</th><th>kategori</th><th width='105px'>Pilihan</th></tr>
            ...
            $tampil = mysqli_query(KONEKSI, 'SELECT * FROM refkategori ORDER BY id_kategori DESC ');
            ...
            while ($r = mysqli_fetch_array($tampil)) {
                ...
                <td><a href=?modul=kategori&act=edit&id=$r[id_kategori]>
		    <input type=button class='btn btn-success btn-mini' value='Edit'/></a>
	                  <a href=$aksi?modul=kategori&act=hapus&id=$r[id_kategori]>
  ...
  <hr>
        <form class='form-horizontal' method=POST action='$aksi?modul=kategori&act=input'>
        ...
        case 'edit':
            $edit = mysqli_query(KONEKSI, "SELECT * FROM refkategori WHERE id_kategori ='$_GET[id]'");

            $r = mysqli_fetch_array($edit);

            echo "
		  <div class='span12 kotak' >
  <h4 align=center>Edit kategori</h4>
  <hr>
        <form class='form-horizontal' method='POST' action='$aksi?modul=kategori&act=update'>

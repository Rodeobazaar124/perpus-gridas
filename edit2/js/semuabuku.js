...
function getaBookList() {
	$('#busy').show();
	$.getJSON(serviceURL + 'semuabuku.php', function(data) {
		$('#busy').hide();
		$('#daftarbuku li').remove();
		b = data.items;
		$.each(b, function(index, c) {
			$('#daftarbuku').append('<li class="img"><a href="detailbuku.html?id=' + c.id_buku + '">' +
				  '<img src="img/sampul.jpg" class="list-icon" width="70px"/>' +
					'' + c.judul + '</br>' +
